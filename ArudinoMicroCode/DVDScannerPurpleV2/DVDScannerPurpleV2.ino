...
// to the pins used:
const int potTopLeft =
	A8; // Analog input pin that the potentiometer is attached to
const int potTopRight =
	A9; // Analog input pin that the potentiometer is attached to
const int potBottomLeft =
	A10; // Analog input pin that the potentiometer is attached to
const int potBottomRight =
	A11; // Analog input pin that the potentiometer is attached to
...
int potValueRaw1, potValueRaw2, potValueRaw3,
	potValueRaw4 = 0; // value read from the pot
...
void loop()
{
	...
	if (j == (csize * 2)) {
		...
		control_data_out[1] = (potValue2 << 2) + (potValue4 >> 2) +
				      focus_shift; //Drive reading head focus
		...
		control_data_out[7] =
			control_data_in[0] * 256 + control_data_in[1];
		control_data_out[8] =
			control_data_in[2] * 256 + control_data_in[3];
		...
		if (control_data_in[13] == 1)
			y = 0;

		if (control_data_in[15] > 1)
			digitalWrite(SEL1, HIGH);
		else
			digitalWrite(SEL1, LOW);
		if ((control_data_in[15] % 2) == 0)
		...
		for (int i = 0; i < sizeX; i++) {
			...
			mcpDac2BSend(posX + ((-(sizeX / 2) + x) *
					     fact)); //Drive reading head X
	...
	if (x < sizeX) {
		...
		if (mode == 0)
			mcpDac1BSend(posY + ((-(sizeY / 2) + y) *
					     fact)); //Drive stage Y
		if (mode == 1)
			mcpDac2ASend(4096 - y * 20);
	...
	if ((potValueRaw1 - potValue1) > 2) {
		potValue1 = potValueRaw1 - 2;
	} // add hysteresis
	if ((potValue1 - potValueRaw1) > 2) {
		potValue1 = potValueRaw1 + 2;
	}

	if ((potValueRaw2 - potValue2) > 2) {
		potValue2 = potValueRaw2 - 2;
	}
	if ((potValue2 - potValueRaw2) > 2) {
		potValue2 = potValueRaw2 + 2;
	}

	if ((potValueRaw3 - potValue3) > 2) {
		potValue3 = potValueRaw3 - 2;
	}
	if ((potValue3 - potValueRaw3) > 2) {
		potValue3 = potValueRaw3 + 2;
	}

	if ((potValueRaw4 - potValue4) > 2) {
		potValue4 = potValueRaw4 - 2;
	}
	if ((potValue4 - potValueRaw4) > 2) {
		potValue4 = potValueRaw4 + 2;
	}
	...
	if (mode == 0)
		mcpDac2ASend((potValue2 << 2) + (potValue4 >> 2) +
			     focus_shift); //Drive reading head focus
